...
export interface ICommonNativeHostService {
	...
	/**
	 * Adds an "Open with Dida" entry to the Windows Explorer context menu for
	 * files, folders, and the folder background (per-user registry, no
	 * elevation required). Windows only.
	 */
	installShellContextMenu(): Promise<void>;

	/**
	 * Appends the application's `bin` folder to the user PATH environment
	 * variable (per-user, no elevation required). Windows only.
	 */
