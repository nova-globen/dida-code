...
export class NativeHostMainService extends Disposable implements INativeHostMainService {
	...
	async addToPath(windowId: number | undefined): Promise<void> {
		if (!isWindows) {
			return;
		}

		const binFolder = join(dirname(process.execPath), 'bin');
		// [Environment]::SetEnvironmentVariable writes the user PATH registry
		// value without truncation and broadcasts WM_SETTINGCHANGE so new
		// shells pick it up.
		const script = `$p = [Environment]::GetEnvironmentVariable('Path', 'User'); if (-not $p) { $p = '' }; if (($p -split ';') -notcontains '${binFolder}') { [Environment]::SetEnvironmentVariable('Path', ($p.TrimEnd(';') + ';${binFolder}'), 'User') }`;
		await promisify(exec)(`powershell -NoProfile -NonInteractive -Command "${script.replace(/"/g, '\\"')}"`);
	}

	async installShellContextMenu(windowId: number | undefined): Promise<void> {
		if (!isWindows) {
			return;
		}

		const exe = process.execPath;
		const label = this.productService.nameLong;
		const keys: [string, string][] = [
			[`HKCU\\Software\\Classes\\*\\shell\\${this.productService.nameShort}`, '%1'],
			[`HKCU\\Software\\Classes\\Directory\\shell\\${this.productService.nameShort}`, '%V'],
			[`HKCU\\Software\\Classes\\Directory\\Background\\shell\\${this.productService.nameShort}`, '%V']
		];

		for (const [key, argument] of keys) {
			await promisify(exec)(`reg add "${key}" /ve /d "Open with ${label}" /f`);
			await promisify(exec)(`reg add "${key}" /v "Icon" /d "${exe}" /f`);
			await promisify(exec)(`reg add "${key}\\command" /ve /d "\\"${exe}\\" \\"${argument}\\"" /f`);
		}
	}
	...
	async openDevTools(windowId: number | undefined, options?: Partial<OpenDevToolsOptions> & INativeHostOptions): Promise<void> {
		const window = this.windowById(options?.targetWindowId, windowId);
		// dock devtools to the bottom of the window by default
		window?.win?.webContents.openDevTools(options?.mode ? { mode: options.mode, activate: options.activate } : { mode: 'bottom' });
	}
