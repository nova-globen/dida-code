...
class OpenNewsletterSignupUrlAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('newsletterSignup', 'Signup for the Dida Newsletter'),
			category: Categories.Help,
			f1: true
		});
	}
...
class OpenLicenseUrlAction extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// Prefer the license file bundled with the application
		if (!isWeb && productService.licenseFileName) {
			const licenseResource = URI.joinPath(FileAccess.asFileUri(''), '..', productService.licenseFileName);
			if (await fileService.exists(licenseResource)) {
				await editorService.openEditor({ resource: licenseResource, options: { pinned: true } });
				return;
			}
		}
...
// The accessibility walkthrough and chat are removed in this fork, so their
// help menu entries are not registered.
