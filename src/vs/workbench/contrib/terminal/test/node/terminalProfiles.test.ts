...
suite('Workbench - TerminalProfiles', () => {
	...
	suite('detectAvailableProfiles', () => {
		if (isWindows) {
			test('should ignore configured non-PowerShell profiles', async () => {
				...
				profilesEqual(profiles, []);
			});
			test('should expose PowerShell 7 as the only detected Windows profile', async () => {
				const pwshSourcePaths = [
					'C:\\Program Files\\PowerShell\\7\\pwsh.exe'
				];
				const fsProvider = createFsProvider(pwshSourcePaths);
				const configurationService = new TestConfigurationService({ terminal: { integrated: { useWslProfiles: false } } });
				const profiles = await detectAvailableProfiles(undefined, undefined, true, configurationService, process.env, fsProvider, undefined, undefined, pwshSourcePaths);
				const expected = [
					{ profileName: 'PowerShell', path: 'C:\\Program Files\\PowerShell\\7\\pwsh.exe', isAutoDetected: true, isDefault: true }
				];
			...
			suite('pwsh source detection/fallback', () => {
				...
				test('should not fall back to Windows PowerShell', async () => {
					const pwshSourcePaths = [
