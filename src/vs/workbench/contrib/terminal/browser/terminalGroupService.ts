...
import { ITerminalGroup, ITerminalGroupService, ITerminalInstance, ITerminalPanelOverride } from './terminal.js';
...
export class TerminalGroupService extends Disposable implements ITerminalGroupService {
	...
	private _panelOverride: ITerminalPanelOverride | undefined;
	get panelOverride(): ITerminalPanelOverride | undefined { return this._panelOverride; }
	set panelOverride(value: ITerminalPanelOverride | undefined) {
		this._panelOverride = value;
	...
	hidePanel(): void {
		if (this._panelOverride) {
			// the terminal home tab cannot be hidden
			return;
		}
	...
	async showPanel(focus?: boolean): Promise<void> {
		if (this._panelOverride) {
			await this._panelOverride.reveal(focus);
			if (focus) {
				await timeout(0);
				await this.activeInstance?.focusWhenReady(true);
			}
			return;
		}
	 ...
	 */
	updateVisibility() {
		const visible = this._panelOverride ? this._panelOverride.isVisible() : this._viewsService.isViewVisible(TERMINAL_VIEW_ID);
