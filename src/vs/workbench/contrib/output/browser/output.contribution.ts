...
/**
 * Opens the Output view in a narrow panel the first time a workspace is
 * opened. Afterwards the user's own panel layout is preserved.
 */
class DefaultOutputPanelContribution implements IWorkbenchContribution {

	private static readonly SHOWN_KEY = 'dida.defaultOutputPanelShown';
	...
	constructor(
		...
		if (storageService.getBoolean(DefaultOutputPanelContribution.SHOWN_KEY, StorageScope.WORKSPACE, false)) {
			return;
		}
		storageService.store(DefaultOutputPanelContribution.SHOWN_KEY, true, StorageScope.WORKSPACE, StorageTarget.MACHINE);
		viewsService.openView(OUTPUT_VIEW_ID, false).then(() => {
			const size = layoutService.getSize(Parts.PANEL_PART);
			layoutService.setSize(Parts.PANEL_PART, { width: size.width, height: DefaultOutputPanelContribution.PANEL_HEIGHT });
		});
	}
}

Registry.as<IWorkbenchContributionsRegistry>(WorkbenchExtensions.Workbench).registerWorkbenchContribution(DefaultOutputPanelContribution, LifecyclePhase.Restored);
