...
export function createApiFactoryAndRegisterActors(accessor: ServicesAccessor): IExtensionApiFactory {
	...
	const expected = Object.values<ProxyIdentifier<any>>(ExtHostContext).filter(context => context !== ExtHostContext.ExtHostMcp);
	...
	return function (extension: IExtensionDescription, extensionInfo: IExtensionRegistries, configProvider: ExtHostConfigProvider): typeof vscode {
		...
		const lm: typeof vscode.lm = {
			...
			registerMcpServerDefinitionProvider(id, provider) {
				...
				throw new Error('MCP server definition providers are disabled in Code Light.');
			},
