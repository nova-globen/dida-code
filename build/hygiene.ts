 ...
 * Lightweight forks without the Copilot extension skip this check.
		...
		new Promise<VinylFile | null>((c, e) => {
			...
			fs.stat(fullPath, (err, stat) => {
				...
				cp.exec(
					...
					(err, out) => {
						if (err) {
							if (err.message.includes('not in the working tree') || err.message.includes('exists on disk, but not in')) {
								// file exists on disk but was removed from the
								// index (git rm --cached): nothing to check
								return c(null);
							}
							return e(err);
						}
